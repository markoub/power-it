...
async def convert_pptx_to_png(pptx_path: str, output_dir: Optional[str] = None) -> List[str]:
    ...
    Args:
        ...
        output_dir: Directory to store the PNG files (defaults to same directory as PPTX)
    ...
    # Changed 'libreoffice' to 'soffice'
    ...
    try:
        ...
        if process.returncode != 0:
            print(f"Error converting PPTX to PDF: {stderr.decode()}")
            return []
        
        # Check if PDF was created
        if not os.path.exists(pdf_path):
            print(f"PDF file not created: {pdf_path}")
            return []
            
        print(f"PDF file created: {pdf_path}")
        
        # Now convert PDF to PNG images using Ghostscript
        png_base_path = os.path.join(output_dir, f"{basename}")
        gs_cmd = [
            "gs", "-dSAFER", "-dBATCH", "-dNOPAUSE", "-sDEVICE=pngalpha",
            "-r144", f"-sOutputFile={png_base_path}-%03d.png", pdf_path
        ]
        ...
        print(f"Ghostscript stdout: {stdout.decode() if stdout else 'None'}")
        print(f"Ghostscript stderr: {stderr.decode() if stderr else 'None'}")
        print(f"Ghostscript exit code: {process.returncode}")
        
        if process.returncode != 0:
            print(f"Error converting PDF to PNG: {stderr.decode()}")
            return []
        
        # Get a list of all generated PNG files
        png_files = [
            os.path.join(output_dir, f)
            for f in os.listdir(output_dir)
            if f.startswith(f"{basename}-") and f.endswith(".png")
        ]
        
        # Sort the PNG files by slide number
        png_files.sort()
